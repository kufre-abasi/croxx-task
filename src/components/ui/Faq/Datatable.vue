<template>
    <div>
        <table class="table-auto w-full">
            ...
            <tbody v-if="rows.data.length" >
                <tr v-for="faq in rows.data" :key="faq" class="border-b text-[14px]">
                    <td class="text-left py-4 pl-4 pr-8">{{ faq.title }}</td>
                    <td class="w-72  py-4 pl-4 pr-8">{{  faq.body }}</td>
                    <td>{{ dayjs(faq.created_at).format("DD MMMM YYYY HH:mm:ss") }}</td>
                    <td class="text-center cursor-pointer relative pr-4">
                        <div  @click="toggleDropdown(faq)" class="flex justify-center">
                            <ThreeDotsIcon />
                        </div>
                        
                        <Dropdown
                            v-if="showDropdownItem.id == faq.id && showDropdownToggle"  
                            :showDropdown="showDropdownItem.id == faq.id && showDropdownToggle" 
                            :link="false"
    ...
    <Modal v-if="showDeleteModal" title="Delete FAQ" @closeModal="toogleDeleteModal">
        <div class="mb-4">
            Are you sure you want to delete this faq? once deleted it cannot be recovered
        </div>
        <div class="flex justify-end gap-4">
            <div>
                <button @click="deleteFaq" class="bg-red-500 text-gray-100 px-4 py-2 rounded">
...
<script setup >
    ...
    const clickedItem = (link) => {
        if(link.context == "Update"){
            router.push({
                name: 'FAQUpdate',
                params:{
                    id: showDropdownItem.value.id
                }
            })
        }

        if(link.context == "Delete"){
            toogleDeleteModal()
        }
    }
    ...
    const deleteFaq = async () => {
        ...
        try {
            console.log(deleteItem.value.id)
            await faqService.deleteFaq(deleteItem.value.id)
            toogleDeleteModal();
            allFaqs.value.data = allFaqs.value.data.filter(item => item.id != deleteItem.value.id);
