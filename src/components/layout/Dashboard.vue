...
<template>
  <div class="bg-[#e5e5e5]">
    <div class="lg:flex">
      <div v-if="closeBackdrop" @click.self="toggle" class="lg:hidden z-20 menubg fixed top-0 left-0 w-full">
        <div :class="{'animate__animated animate__slideOutLeft':!closeNav}" class="w-2/3 animate__animated animate__slideInLeft">
          <BaseNavbar />
        </div>
      </div>
      <div class="hidden lg:block flex-shrink-0 md:w-[320px] ">
        <BaseNavbar />
      </div>
      <div class="p-0 flex-grow">
        ...
        <div class="lg:hidden">
          <div  class="logo">
            <router-link to="/" ><img src="@/assets/img/logo.svg" class="h-8 w-24" alt="Kavlr logo"/></router-link>
          </div>
        </div>
          <h3 class="lg:block text-[#001727] font-Satoshi500 text-[24px] leading-[32.4px] uppercase">{{ title }}</h3>
        <div class=" text-right justify-end">
          <div class="flex justify-between gap-3 items-center ml-auto">
            <div class="profile__dropdown relative">
              <div
                class="profile__dropdown-text items-center flex p-2 rounded-3xl">
                <img  @click="toogleDropdown()"
                  class=" md:block"
                  src="@/assets/img/profile-icon.png"
                  alt="" />
                <span class="user-name hidden lg:block font-Satoshi700 text-[16px] leaading-[24px]"></span>
                <svg
