...
const updateBlog = async () => {
    ...
    try {
        ...
        let res = await blogService.updateBlog(payload.value, blog.value.slug)
    } catch (error) {
        console.log(error)
    }
    finally{
...
const getSingleBlog = async () => {
    ...
    try{
        blog.value = await blogService.getSingleBlog(route.params.id)
        payload.value.title = blog.value.title
        payload.value.blog_description = blog.value.blog_description
        payload.value.body = blog.value.body
        payload.value.cover_image = previewImageUrl.value = blog.value.cover_image,
        payload.value.blog_category = blog.value.blog_category
    }
    catch (error){
        console.log(error)
    }
        ...
        <div class="lg:px-6 px-3  pt-8 grid grid-rows gap-4">
            ...
            <div class="bg-white py-8 rounded-[8px] min-h-[100vh] relative px-2">
                <div class="flex lg:flex-row flex-col gap-3 px-4 justify-between mb-4">
                    <h3 class="font-Satoshi500 text-[24px] place-self-center leading-[32.4px] text-[#001727]">Update Blog</h3>
                </div>

                <form @submit.prevent="updateBlog" method="post" class="px-4">
                    <div class="mb-6">
                    ...
                    <div class="flex justify-end">
                        <button type="submit" class="btn my-4 !px-16">
                            ...
                            <span v-if="!buttons[0]">Update blog</span>
                        </button>    
                    </div>
